...
function main()
    reps = [SUNIrrep(0, 0, 0), SUNIrrep(1, 0, 0), SUNIrrep(1, 1, 0)]
    fk = map(r -> exp(-β) * sum(det([besseli(weight(r)[j] + i - j + q, β / N) for i in 1:N, j in 1:N]) for q in (-absq_max):absq_max), reps)
    indices = NTuple{5, SUNIrrep{N}}[]
                                            ...
                                            if haskey(b5, r5)
                                                for r6 in keys(a6)
                                                    if haskey(b6, r6)
                                                        ...
                                                        prod(r -> fk[findfirst(isequal(r), reps)], [r121, r122, r123, r124, r231, r232, r233, r234, r311, r312, r313, r314]) ^ (1 / 4) / sqrt(prod(dim, [r1, r2, r3, r4, r5, r6]))
                                                    end
                                                end
                                            end
                                        end
